...
import {
  LayoutDashboard, FileText, File, FormInput, BarChart3, Users,
  ChevronLeft, ChevronRight, Settings, Menu, LogOut, User, UsersRound,
} from "lucide-react";
import {
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`fixed z-50 md:relative flex flex-col h-full bg-sidebar text-sidebar-foreground border-r border-sidebar-border transition-all duration-300 ${collapsed ? "w-[68px]" : "w-[240px]"} ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="flex items-center gap-3 px-4 h-16 border-b border-sidebar-border">
          <div className="w-8 h-8 rounded-lg bg-sidebar-primary flex items-center justify-center text-sidebar-primary-foreground font-bold text-sm shrink-0">F</div>
          {!collapsed && <span className="font-semibold text-sidebar-accent-foreground text-lg tracking-tight">Foreware</span>}
        </div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold text-foreground">
              {navItems.find((i) => i.path === location.pathname)?.label
                || { "/settings": "Settings", "/profile": "Profile", "/users": "Users & Roles" }[location.pathname]
                || "Dashboard"}
            </h1>
          </div>
          <DropdownMenu>
